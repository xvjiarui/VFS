...
from .image_utils import get_cropped_input
...
def crop_and_resize(img,
    ...
    if faster:
        ...
        size = max(2, size)
        xyxy = xywh_to_xyxy([center[1], center[0], size, size])
        avg_color = np.mean(img, axis=(0, 1), dtype=float)

        # Use faster version from https://github.com/danielgordon10/vince/blob/43c4c2c528a7a285de8748d28e653f6183fef086/siamfc_pytorch/ops.py#L116  # noqa
        patch = get_cropped_input(img, xyxy, 1, out_size, interp, avg_color)[0]
        return patch
    # convert box to corners (0-indexed)
    size = round(size)
